...
void main(void)
{
    ...
    if (uAlphaMask != 0 && baseColor.a < uAlphaCutoff) discard;
    ...
    vec3 dirDiff = diffAlbedo * uDirectionalLightColor * uDirectionalLightIntensity * NdotLd * shadow;
    vec3 dirSpec = F0 * pow(max(dot(N, Hd), 0.0), shininess) * uDirectionalLightColor * uDirectionalLightIntensity * NdotLd * shadow;
    ...
    for (int i = 0; i < uNumPointLights; i++)
    {
        ...
        vec3 L = toLight / dist;
        float rangeAtten = (uPointLights[i].applyAttenuation != 0)
                           ? smoothstep(uPointLights[i].range, 0.0, dist)
                           : 1.0;
    ...
    if (uEmissiveEnabled != 0) litColor += emissive;

    FragColor = vec4(litColor, baseColor.a);
    EmissiveOut = (uEmissiveEnabled != 0) ? vec4(emissive, 1.0) : vec4(0.0);
    GodRayOut = (uGodRayEmitter != 0 && uEmissiveEnabled != 0) ? vec4(emissive, 1.0) : vec4(0.0);
}
