...
int initialize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"void main(void)\n"
		"{\n"
			"out_localPosCube = aPosition.xyz;\n"
	...
	// local position
	glGenTextures(1, &gLocalPos);
	glBindTexture(GL_TEXTURE_2D, gLocalPos);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, WIN_WIDTH, WIN_HEIGHT, 0, GL_RGB, GL_FLOAT,NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, gLocalPos, 0);
...
int initialize_quad(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"in vec2 aTexCoord;\n"
		"out vec2 TexCoord;\n"
		"out vec3 out_localPosQuad;\n"
		"void main(void)\n"
		"{\n"
			"TexCoord = aTexCoord;\n"	
			"gl_Position = aPosition;\n"
			"out_localPosQuad = aPosition.xyz;\n"
		"}\n"; 
	...
	const GLchar *fargamentShaderSourceCode =
		"#version 460 core\n"
		"in vec2 TexCoord;\n"
		"in vec3 out_localPosQuad;\n"
		"uniform  sampler2D gPosition;\n"
		"uniform  sampler2D gNormal;\n"
		"uniform  sampler2D gAlbedo;\n"
		"uniform  vec3 lightPos;\n"
		"uniform  sampler2D decalTex;\n"
		"uniform sampler2D gLocalPos;\n"
		"out vec4 FragColor;\n"
		"void main(void)\n"
		"{\n"
			...
			"{\n"
				"discard;\n" //(it removes empty back ground space)
			"}\n"
			"vec3 localPos = texture(gLocalPos, TexCoord).rgb;\n"
			// 1. Calculate UVs
			"vec2 decalUV = localPos.xy * 0.5 + 0.5;\n"
			// 2. Fetch both textures
			"vec3 color = texture(gAlbedo, TexCoord).rgb;\n"
			"vec4 decal = texture(decalTex, decalUV);\n"
			// 3. Create a custom mix factor instead of relying on missing alpha
			// Prevent stretching by only applying to the front face (+Z)
			"//vec3 norm = normalize(texture(gNormal, TexCoord).rgb);\n"
			"float mixFactor =  smoothstep(0.3, 0.9, norm.z);\n"
			"if (localPos.z < 0.99) {\n"
				"mixFactor = 0.0;\n"
			"}\n"
			// Don't 'discard' out-of-bounds UVs, just set mixFactor to 0
			"if(decalUV.x < 0.0 || decalUV.x > 1.0 || decalUV.y < 0.0 || decalUV.y > 1.0)\n"
			"{\n"
				"mixFactor = 0.0;\n"
			"}\n"
			// Assuming the background of your decal BMP is black (0,0,0). 
			// If it's a different color (like white), change this logic to target that color.
			"if(length(decal.rgb) < 0.1) {\n"
				"mixFactor = 0.0;\n"
			"}\n"

			// 4. Calculate lighting and final color
			"vec3 lightDir = normalize(lightPos - fragPos);\n"
			"float diffLight = max(dot(norm, lightDir), 0.0);\n"
			
			"vec3 finalColor = mix(color, decal.xyz, mixFactor);\n"
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, gLocalPos);
	glUniform1i(glGetUniformLocation(shaderProgramObject_quad,"gLocalPos"),4);
