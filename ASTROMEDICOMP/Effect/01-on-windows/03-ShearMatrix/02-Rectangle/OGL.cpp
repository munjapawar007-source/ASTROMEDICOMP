...
int initialize(void)
{
	...
	// COLOR
	glGenBuffers(1, &vbo_color); // all arrays associate with vao
...
void display(void)
{
	...
	vmath::mat4 shearMatrix = vmath::mat4::identity();

	shearMatrix[0][0] = 1.0f;
	shearMatrix[0][1] = cos(angleSh);
	shearMatrix[0][2] =  cos(angleSh);
	...
	shearMatrix[2][0] = 0.0f;
	shearMatrix[2][1] = 0.0f;
	shearMatrix[2][2] = 1.0f;
	shearMatrix[2][3] = 0.0f;

	shearMatrix[3][0] = 0.0f;
	shearMatrix[3][1] = 0.0f;
	...
	modelViewMatrix = translationMatrix * shearMatrix;
