...
int initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(rectanglePosition_TexCoords), rectanglePosition_TexCoords, GL_STATIC_DRAW);
...
void drawCircle(void)
{
	...
	int index = 0;
	circle_position = (float *)malloc(sizeof(float) * COUNT * 3);
	...
	printf("circle_position:%p\n", &circle_position);

	for (float i = 0.0f; i <= 360.0f; i += 0.20)
	{
		x = 1.0f * cos(i * rad);
		y = 1.0f * sin(i * rad);
		...
		glBufferData(GL_ARRAY_BUFFER, COUNT * 3 * sizeof(float), circle_position, GL_STATIC_DRAW);
		glVertexAttribPointer(AMC_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);
		glEnableVertexAttribArray(AMC_ATTRIBUTE_POSITION);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
		glBindVertexArray(0);
	...
	free(circle_position);
	circle_position = NULL;
}
