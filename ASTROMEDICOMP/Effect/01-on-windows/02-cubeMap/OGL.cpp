...
int initialize(void)
{
	...
	viewUniform = glGetUniformLocation(shaderProgramObject, "view");
	projectionUniform = glGetUniformLocation(shaderProgramObject, "projection");
	textureSkyboxUniform = glGetUniformLocation(shaderProgramObject, "skybox");
	...
	glVertexAttribPointer(AMC_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), NULL);
	...
	for (unsigned int i = 0; i < 6; i++)
	{
		...
		if (data)
		{
			...
			fprintf(gpFile, "STBI LOAD SUCCESS.\n", textureNames[i]);
		}
		else
		{
			fprintf(gpFile, "Cubemap tex failed to load at path: %s\n", textureNames[i]);
			stbi_image_free(data);
		}
	}
...
void display(void)
{
	...
	translationMatrix = vmath::translate(0.0f, 0.10f, -0.0f);
	...
	modelViewMatrix = translationMatrix * scaleMatrix * rotationMatrixY; // order is important
	...
	glUniformMatrix4fv(
		viewUniform,	// shader madhla uniform tumhi tumchya kontya var madhe ghetlay
		1,				// kiti matrices pathavtay
		GL_FALSE,		// matrix transpose karaychay ka? like mirror view
		modelViewMatrix // konta matrix?
	);
	glUniformMatrix4fv(projectionUniform, 1, GL_FALSE, ModelViewProjectionMatrix);

	// for texture
	glActiveTexture(GL_TEXTURE0);
	//glBindTexture(GL_TEXTURE_CUBE_MAP, textureID);
	glUniform1i(textureSkyboxUniform, 0);
...
void update(void)
{
	...
	angleCube_MP = angleCube_MP - 0.005f;
