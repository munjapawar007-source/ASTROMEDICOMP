...
int initialize(void)
{
	...
	const GLchar *fargamentShaderSourceCode =
		...
		"  view = view / view.w;\n" \
		"  vec3 worldRay = normalize((uInverseViewMatrix * vec4(view.xyz, 0.0)).xyz);\n" \
		"  return worldRay;\n" \
		"}\n" \
...
void display(void)
{
	...
	vmath::vec3 sunDirection   = vmath::normalize(vmath::vec3(0.0f, 1.0f, 0.0f));
