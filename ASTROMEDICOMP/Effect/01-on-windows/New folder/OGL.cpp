...
GLuint ldUniform_sphere = 0; // Light Diffuse
GLuint lsUniform_sphere = 0; // Light Diffuse
...
int initialize(void)
{
	...
	glClearColor(1.0f, 1.0f, 1.0f, 1.0f); // for beauty use white color
...
BOOL createAndPrepareFBOForDrawing(GLint textureWidth, GLint textureHeight)
{
	...
	{
		fprintf(gpFile, "FBO size exceeds max renderbuffer size\n");
		return(FALSE);
	}
	...
	glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,textureWidth,textureHeight,0,GL_RGB,GL_UNSIGNED_SHORT_5_6_5,NULL); //5_6_5 for human unstood greenary
...
void display(void)
{
	...
	scaleMatrix = vmath::scale(1.0f,1.0f, 1.f);
