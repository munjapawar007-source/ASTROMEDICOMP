      ...
      implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener 
{
   ...
   private int CHECKERBOARD_WIDTH = 64;
   private int CHECKERBOARD_HEIGHT = 64;
   byte CheckerboardImage[] = new byte[CHECKERBOARD_HEIGHT * CHECKERBOARD_WIDTH * 4];
   private float rectanglePosition[] = new float[12];
   ...
   private int initialize(GL10 gl) 
   {
      ...
      GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, 4 * 3 * 4, null, GLES32.GL_STATIC_DRAW);
      ...
      // 1) set buffer size (36 byte ) 
          ByteBuffer byteBuffer = ByteBuffer.allocateDirect(rectangle_Texcoords.length * 4);
      ...
      // 4) fill array of rectangle_ position
         rectangleTexcoordBuffer.put(rectangle_Texcoords);
      // 5)  set array to 0th position for repeatable use
         rectangleTexcoordBuffer.position(0);

      GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, rectangle_Texcoords.length * 4, rectangleTexcoordBuffer, GLES32.GL_STATIC_DRAW);
   ...
   {
      // create procdural image
      MakeCheckerboard();

      // just like vbo create empty image buufer
      ByteBuffer byteBuffer = ByteBuffer.allocateDirect(CHECKERBOARD_WIDTH * CHECKERBOARD_HEIGHT * 4);
      byteBuffer.order(ByteOrder.nativeOrder());
      byteBuffer.put(CheckerboardImage);
      byteBuffer.position(0);

      //create bitmp image in the memory from the above buufer
   ...
   private void MakeCheckerboard()
   {
      ...
      for(i = 0; i < CHECKERBOARD_HEIGHT; i++)
      {
         ...
         {
            c = (( i & 8) ^ (j & 8)) * 255;
            CheckerboardImage[(i * 64 + j) * 4 + 0] = (byte)c;
            CheckerboardImage[(i * 64 + j) * 4 + 1] = (byte)c;
            CheckerboardImage[(i * 64 + j) * 4 + 2] = (byte)c;
            CheckerboardImage[(i * 64 + j) * 4 + 3] = (byte)0xff;
         }
      }
   }
   ...
   private void Display() 
   {
     ...
     float ModelViewProjectionMatrix[] = new float[16];
      ...
      rectanglePosition[6] = -2.0f;
      rectanglePosition[7] = -1.0f;
      rectanglePosition[8] = 0.0f;
      rectanglePosition[9] = 0.0f;
      rectanglePosition[10] = -1.0f;
      rectanglePosition[11] = 0.0f;
      ...
      // 4) fill array of rectangle_ position
         rectanglePositionBuffer.put(rectanglePosition);
      // 5)  set array to 0th position for repeatable use
         rectanglePositionBuffer.position(0);
     
      GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo[0]);
      GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, rectanglePosition.length * 4, rectanglePositionBuffer, GLES32.GL_DYNAMIC_DRAW);
      GLES32.glVertexAttribPointer(MyAttributs.MSP_ATTRIBUTE_POSITION, 3, GLES32.GL_FLOAT, false, 0, 0); //jithe mi data bharla tithe ja ani tithun don don data bajula kar 
      GLES32.glEnableVertexAttribArray(MyAttributs.MSP_ATTRIBUTE_POSITION); // data enable kar
      GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
      ...
      rectanglePosition[11] = -1.41421f;
        ...
        // create a native buffer suitable for native IO but for java
      // 1) set buffer size (36 byte ) 
         byteBuffer = ByteBuffer.allocateDirect(rectanglePosition.length * 4);
      // 2) set order left to right OR right to left
         byteBuffer.order(ByteOrder.nativeOrder());
      // 3) create which type is data
   ...
   private void Uninitialize() 
   {
      ...
      {
         GLES32.glDeleteBuffers(1,texture_Checkerboard,0);
