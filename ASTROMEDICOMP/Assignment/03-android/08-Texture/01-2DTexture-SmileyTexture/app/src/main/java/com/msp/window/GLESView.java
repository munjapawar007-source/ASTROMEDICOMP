      ...
      implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener 
{
   ...
   private int initialize(GL10 gl) 
   {
      ...
      // 2) set order left to right OR right to left
          byteBuffer.order(ByteOrder.nativeOrder());
      // 3) create which type is data
         FloatBuffer rectangleTexcoordsBuffer = byteBuffer.asFloatBuffer();
      // 4) fill array of rectangle_ position
         rectangleTexcoordsBuffer.put(rectangle_Texcoords);
      // 5)  set array to 0th position for repeatable use
          rectangleTexcoordsBuffer.position(0);
      GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, rectangle_Texcoords.length * 4, rectangleTexcoordsBuffer, GLES32.GL_STATIC_DRAW);
