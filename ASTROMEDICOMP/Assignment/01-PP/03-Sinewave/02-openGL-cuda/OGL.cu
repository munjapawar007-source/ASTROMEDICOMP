...
int initialize(void)
{
	...
	// cuda releated device selection
	int devCount = 0;
	cudaResult = cudaGetDeviceCount(&devCount);
	if (cudaResult != cudaSuccess)
	{
		fprintf(gpFile, "cudaGetDevice Failed Due To %s.\n", cudaGetErrorString(cudaResult));
		uninitialize();
		exit(EXIT_FAILURE);
	}
	else if (devCount == 0)
	{
		fprintf(gpFile, "devCount Successed but cuda device not found\n");
		uninitialize();
		exit(EXIT_FAILURE);
	}
	cudaSetDevice(0);
	...
	const GLchar *fargamentShaderSourceCode =
		"#version 460 core\n"
		"out vec4 FragColor;\n"
		"uniform mat4 uColorSinewave;\n"
		"void main(void)\n"
		"{FragColor=vec4(1.0f,0.50f,1.0f,1.0f);\n"
		"}\n";
	...
	// colorSinewaveUniform = glGetUniformLocation(shaderProgramObject, "uMVPMatrix");
	...
	// cuda releted initialization
	cudaResult = cudaGraphicsGLRegisterBuffer(&cuGraphicsResource, vbo_gpu, cudaGraphicsRegisterFlagsWriteDiscard);

	if (cudaResult != cudaSuccess)
	{
		fprintf(gpFile, "cuda GrapicsGl Register Failed .\n");
...
void display(void)
{
	...
	if (bOnGPU)
	{
		...
		// step2: get host compatible mapped pointer for grapics resocres
		...
		// step3: RUN cuda kernel
		dim3 block(8, 8, 1);		// for 8 thread in per block
		...
		// unmap mapped pointer
		cudaResult = cudaGraphicsUnmapResources(1, &cuGraphicsResource, 0);

		// remember pPosition is nothing but our vbo_gpu hence here bind with it
		glBindBuffer(GL_ARRAY_BUFFER, vbo_gpu);
	}
	else
	{
