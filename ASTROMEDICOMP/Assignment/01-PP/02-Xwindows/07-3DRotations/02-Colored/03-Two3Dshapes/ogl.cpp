...
int Initialize(void)
{
    ...
    glGenBuffers(1, &vbo_color_cube); // all arrays associate with vao
    ...
    // COLOR
    glGenBuffers(1, &vbo_color_pyramid); // all arrays associate with vao
    ...
    perspectiveProjectionMatrix = vmath::mat4::identity(); // this is annalogas to load identity (resize madhil) in ffp
...
void display(void)
{
    ...
    translationMatrix = vmath::translate(0.50f, 0.0f, -1.5f);
    scaleMatrix = vmath::scale(0.2f, 0.2f, 0.2f);
    rotationMatrix = vmath::rotate(angleCube_MP, 0.0f, 1.0f, 0.0f);
    ...
    translationMatrix = vmath::translate(-0.5f, 0.0f, -1.5f);
    scaleMatrix = vmath::scale(0.2f, 0.2f, 0.2f);
    rotationMatrix = vmath::rotate(anglePyramid_MP, 0.0f, 1.0f, 0.0f);
...
void update(void)
{
    ...
    anglePyramid_MP += 0.5;
    if (anglePyramid_MP <= 0.0)
    {
        anglePyramid_MP -= 360.0f;
    }
}
