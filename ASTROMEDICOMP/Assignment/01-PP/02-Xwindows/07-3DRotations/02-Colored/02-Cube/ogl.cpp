...
int Initialize(void)
{
    ...
    glBindAttribLocation(shaderProgramObject, AMC_ATTRIBUTE_COLOR, "aColor"); // 0th index match cpu and gpu side
    ...
    // Position
    glGenBuffers(1, &vbo_position_cube); // all arrays associate with vao_cube
    ...
    glGenBuffers(1, &vbo_color_cube); // all arrays associate with vao_cube
