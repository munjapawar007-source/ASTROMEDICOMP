...
void pushMatrix()
{
    matrixStack.push(currentMatrix);			// add element
}

void popMatrix()
{
    if (!matrixStack.empty())					// check stack is empty or not
    {
        currentMatrix = matrixStack.top();		// show top element (not remove)
        matrixStack.pop();						// remove top element 
    }
}
...
void display(void)
{
    ...
    translationMatrix = vmath::translate(1.5f, -0.10f, 0.0f);
	rotationMatrix = vmath::rotate(90.0f, 0.0f, 1.0f, 0.0f);
    currentMatrix = currentMatrix * translationMatrix * rotationMatrix;

	rotationMatrix = vmath::rotate((float)date, 0.0f, 1.0f, 0.0f);
	currentMatrix = currentMatrix * rotationMatrix; //

	//set Earth Polygons Properties
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    {
        
         scaleMatrix = vmath::scale(0.80f, 0.8f, 0.80f);
        mat4 renderMatrix = currentMatrix * scaleMatrix;

        glUniformMatrix4fv(modelMatrixUniform, 1, GL_FALSE, renderMatrix);
        glUniform3f(materialColorUniform, 0.4f,0.7f,1.0f);
	...
	// moon
    {
        scaleMatrix = vmath::scale(0.19f, 0.20f, 0.19f);
        mat4 renderMatrix = currentMatrix * scaleMatrix;

        glUniformMatrix4fv(modelMatrixUniform, 1, GL_FALSE, renderMatrix);
        glUniform3f(materialColorUniform, 1.0f,0.9f,1.0f);
