...
int initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof((GLfloat)4.0f * 2.0f), NULL, GL_STATIC_DRAW);
...
BOOL LoadGLTexture(GLuint *Texture, TCHAR imageID[])
{
	...
	if (hBitmap)
	{
		...
		// gluBuild2DMipmaps(GL_TEXTURE_2D,3,bmp.bmWidth,bmp.bmHeight,GL_BGR_EXT,GL_UNSIGNED_BYTE,bmp.bmBits); use in FFP
...
void display(void)
{
	...
	modelViewMatrix = translationMatrix; // order is important
	...
	GLfloat rectangleTexcord[8];

	if (numKey == 1)
	{
		rectangleTexcord[0] = 1.0f;
		rectangleTexcord[1] = 1.0f;
		rectangleTexcord[2] = 0.0f;
		rectangleTexcord[3] = 1.0f;
		rectangleTexcord[4] = 0.0f;
		rectangleTexcord[5] = 0.0f;
		rectangleTexcord[6] = 1.0f;
		rectangleTexcord[7] = 0.0f;
		glUniform1i(keyPressUniform, 1);
	}
	else if (numKey == 2)
	{
		rectangleTexcord[0] = 0.5;
		rectangleTexcord[1] = 0.5;
		rectangleTexcord[2] = 0.0;
		rectangleTexcord[3] = 0.5;
		rectangleTexcord[4] = 0.0;
		rectangleTexcord[5] = 0.0;
		rectangleTexcord[6] = 0.5;
		rectangleTexcord[7] = 0.0;
		glUniform1i(keyPressUniform, 2);
	}
	else if (numKey == 3)
	{

		rectangleTexcord[0] = 3.0;
		rectangleTexcord[1] = 3.0;
		rectangleTexcord[2] = 1.0;
		rectangleTexcord[3] = 3.0;
		rectangleTexcord[4] = 1.0;
		rectangleTexcord[5] = 1.0;
		rectangleTexcord[6] = 3.0;
		rectangleTexcord[7] = 1.0;
	...
	else if (numKey == 4)
	{
		...
		rectangleTexcord[0] = 0.5;
		rectangleTexcord[1] = 0.5;
		rectangleTexcord[2] = 0.51;
		rectangleTexcord[3] = 0.5;
		rectangleTexcord[4] = 0.51;
		rectangleTexcord[5] = 0.51;
		rectangleTexcord[6] = 0.5;
		rectangleTexcord[7] = 0.5;
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_TexCoordRectangle);
	glBufferData(GL_ARRAY_BUFFER, sizeof(rectangleTexcord), rectangleTexcord, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(AMC_ATTRIBUTE_TEXCOORD, 2, GL_FLOAT, GL_FALSE, 0, NULL); // jithe data bharla tithe ja ani tithun 2 2 data bajula kar
	glEnableVertexAttribArray(AMC_ATTRIBUTE_TEXCOORD);							   // data enable kar
	glBindBuffer(GL_ARRAY_BUFFER, 0);
