...
int initialize(void)
{
	...
	// texcoord
	glGenBuffers(1, &vbo_TexCoord_pyramid); // all arrays associate with vao
	...
	{
		fprintf(gpFile,"stone texture is failed");
		return(-6);
	}

	if(LoadGLTexture(&texture_kundali, MAKEINTRESOURCE(IDBITMAP_KUNDALI)) == FALSE)
	{
		fprintf(gpFile,"kundali texture is failed");
		return(-7);
	}
	//Enable Texturing
...
{
	// code
	anglePyramid_MP = anglePyramid_MP + 0.05f;
