...
GLuint numberOfLineSegmentsUniform = 0;
GLuint numberOfLineStripsUniform = 0;
GLuint lineColorsUniform = 0;
...
int initialize(void)
{
	...
	// layout = arrangment, vertices=4(patch)
	// tessellation shader provide level (inner or outer)
	...
	if (status == GL_FALSE)
	{
		glGetShaderiv(tessellationControlShaderObject, GL_INFO_LOG_LENGTH, &infoLogLenght);
		if (infoLogLenght > 0)
		{
			szInfoLog = (GLchar *)malloc(infoLogLenght * sizeof(GLchar));
			...
			if (szInfoLog != NULL)
			{
				glGetShaderInfoLog(tessellationControlShaderObject, infoLogLenght, NULL, szInfoLog);
				fprintf(gpFile, "Tessellation Control Shader Compilation log = %s", szInfoLog);
				free(szInfoLog);
				szInfoLog = NULL;
			}
		}
	...
	// tessellation madhe draw karaychya goshtila domain manta 
	// 3 domain in tessellation shader Quads,triangle,line
	//  isolines = asha line jya depth la support karta (in Graphics) 
	...
	if (status == GL_FALSE)
	{
		glGetShaderiv(tessellationEvaluationShaderObject, GL_INFO_LOG_LENGTH, &infoLogLenght);
		if (infoLogLenght > 0)
		{
			szInfoLog = (GLchar *)malloc(infoLogLenght * sizeof(GLchar));
			...
			if (szInfoLog != NULL)
			{
				glGetShaderInfoLog(tessellationEvaluationShaderObject, infoLogLenght, NULL, szInfoLog);
				fprintf(gpFile, "Tessellation Evaluation Shader Compilation log = %s", szInfoLog);
				free(szInfoLog);
				szInfoLog = NULL;
			}
		}
	...
	glBindAttribLocation(shaderProgramObject, AMC_ATTRIBUTE_POSITION, "vPosition"); // 0th index match cpu and gpu side
	...
	lineColorsUniform = glGetUniformLocation(shaderProgramObject, "uLineColor");
...
void display(void)
{
	...
	// send line segment as line uniform
	glUniform1i(numberOfLineSegmentsUniform, uiNumberOfLineSegments);
	TCHAR str[225];
	wsprintf(str,TEXT(" MP (numberOfLineSegmentsUniform = %d)"), uiNumberOfLineSegments);
	SetWindowText(ghwnd,str);

	glUniform1i(numberOfLineStripsUniform, 1); //as we have only one line
	glUniform4fv(lineColorsUniform,1,vmath::vec4(1.0f,1.0f,0.0f,1.0f));
